...
USAGE: pproc <inputFile> [outputFile] [-Ddefine1] [-Ddefine2]... [-DdefineN]
...
local openInner = 0
local lines = {}
-- add fake lines from the command line
for i=1,#args
do
  local define = string.match(args[i], "-D(.*)")
  
  if define ~= nil then
    lines[#lines + 1] = "#define "..define.."\n"
  end
end
          ...
          if inStr then
            ...
            if Ihandle then
              ...
              for Iline in Idata:gmatch("([^\n]*)\n") do
                ...
                Ilines[#Ilines + 1] = string.gsub(Iline,"\13","")
              end
