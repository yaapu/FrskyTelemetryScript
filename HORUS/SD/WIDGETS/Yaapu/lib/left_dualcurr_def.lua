...
function panel.draw(widget, x, battId)
  ...
  lcd.drawText(x+1,18,"B1",0+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(x+1,83,"B2",0+CUSTOM_COLOR+SMLSIZE)
  ...
  libs.drawLib.drawNumberWithDim(x+100,16,x+100,26,status.battery[7+1]*(flagUseDecimals and 1 or 0.1),"A",MIDSIZE+RIGHT+CUSTOM_COLOR+(flagUseDecimals and PREC1 or 0),SMLSIZE+CUSTOM_COLOR)
  -- battery 2 current
  flagUseDecimals = status.battery[7+2]*0.1 < 10
  libs.drawLib.drawNumberWithDim(x+100,16+65,x+100,26+65,status.battery[7+2]*(flagUseDecimals and 1 or 0.1),"A",MIDSIZE+RIGHT+CUSTOM_COLOR+(flagUseDecimals and PREC1 or 0),SMLSIZE+CUSTOM_COLOR)
  -- battery 1 capacity bar %
  lcd.setColor(CUSTOM_COLOR, WHITE)
  lcd.drawFilledRectangle(x+10, 45,100,18,CUSTOM_COLOR)
  if perc1 > 50 then
  ...
  lcd.drawGauge(x+10, 45,100,18,perc1,100,CUSTOM_COLOR)
  -- battery 2 capacity bar %
  lcd.setColor(CUSTOM_COLOR, WHITE)
  lcd.drawFilledRectangle(x+10, 45+65,100,18,CUSTOM_COLOR)
  if perc2 > 50 then
    lcd.setColor(CUSTOM_COLOR, lcd.RGB(0,255,0))
  ...
  -- home distance
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(140,140,140))
  lcd.drawText(homeDistX, 150, "HOME("..unitLabel..")", SMLSIZE+RIGHT+CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  local flags = 0
  ...
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(homeDistX, 165, strdist, MIDSIZE+flags+RIGHT+CUSTOM_COLOR)
  ...
  if conf.rangeFinderMax > 0 then
    ...
    lcd.drawText(gpsAltX, 150, "RNG("..unitLabel..")", SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.setColor(CUSTOM_COLOR,utils.colors.red)
    if rng > conf.rangeFinderMax and status.showMinMaxValues == false then
      lcd.drawFilledRectangle(gpsAltX-65, 165+4,65,21,CUSTOM_COLOR)
    end
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawText(gpsAltX, 165, string.format("%.1f",rng*0.01*unitScale), MIDSIZE+RIGHT+CUSTOM_COLOR)
  else
    flags = BLINK
    ...
    lcd.setColor(CUSTOM_COLOR,lcd.RGB(140,140,140))
    lcd.drawText(gpsAltX, 150, "GALT("..unitLabel..")", SMLSIZE+CUSTOM_COLOR+RIGHT)
    local stralt = string.format("%d",alt*unitScale)
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawText(gpsAltX, 165, stralt, MIDSIZE+flags+RIGHT+CUSTOM_COLOR)
  end
end
