...
function layout.draw(widget, customSensors, leftPanel, centerPanel, rightPanel)
  ...
  lcd.drawLine(0, 148, LCD_W, 148, SOLID, CUSTOM_COLOR)
  --]]
  ...
  libs.drawLib.drawRArrow(240,173,19,math.floor(telemetry.homeAngle - telemetry.yaw),CUSTOM_COLOR)
  ...
  if status.batt2sources.fc or status.batt2sources.vs then
    if status.showDualBattery == false then
      ...
      rightPanel[status.currentScreen].draw(widget, 360, 0)
      -- left pane info
      leftPanel[status.currentScreen].draw(widget, 0, 0)
      battIdForPower = 0
    else
      -- dual battery:battery 1 right pane
      rightPanel[status.currentScreen].draw(widget, 360, 1)
      -- dual battery:battery 2 left pane
      rightPanel[status.currentScreen].draw(widget, 0, 2)
    end
  else
  ...
  if conf.enableRPM == 2  or conf.enableRPM == 3 then
    ...
    libs.drawLib.drawBar("rpm1", 10, 165, 90, 24, utils.colors.darkyellow, math.abs(telemetry.rpm1), MIDSIZE)
  end
  -- RPM 2
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  if conf.enableRPM == 3 then
    lcd.drawText(115, 150, "RPM 2", SMLSIZE+CUSTOM_COLOR)
    libs.drawLib.drawBar("rpm2", 115, 165, 90, 24, utils.colors.darkyellow, math.abs(telemetry.rpm2), MIDSIZE)
  end
  -- throttle %
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  ...
  if customSensors ~= nil then
    msgRows = 1
    -- draw custom sensors
    libs.drawLib.drawCustomSensors(0, customSensors, customSensorXY, utils.colors.lightgrey)
  end
  libs.drawLib.drawStatusBar(msgRows)
