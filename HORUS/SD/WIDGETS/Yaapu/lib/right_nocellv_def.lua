...
function panel.draw(widget, x, battId)
  ...
  -- battery voltage
  flags = CUSTOM_COLOR
  libs.drawLib.drawNumberWithDim(x+106,28,x+108, 47, status.battery[4+battId],"V",RIGHT+PREC1+DBLSIZE+CUSTOM_COLOR,SMLSIZE+CUSTOM_COLOR)
  local lx = x+108
  lcd.setColor(CUSTOM_COLOR,utils.colors.white) -- white
  -- battery current
  ...
  if status.showMinMaxValues == true then
    ...
    libs.drawLib.drawVArrow(x+106+11,28 + 3, false,true)
    libs.drawLib.drawVArrow(x+108+11,73 + 10,true,false)
  end

  lcd.setColor(CUSTOM_COLOR, lcd.RGB(140, 140, 140))
  lcd.drawText(x+110, 16, string.format("%s BATT",string.upper(status.battsource)), SMLSIZE+RIGHT+CUSTOM_COLOR)
