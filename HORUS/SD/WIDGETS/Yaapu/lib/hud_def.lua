...
function panel.draw(widget)
  ...
  do
      ...
      if yy >= startY and yy < endY then
        ...
        lcd.drawNumber(127,  yy, j, SMLSIZE+CUSTOM_COLOR)
      end
      ii=ii+1;
  end
  -- altitude
  ...
  do
      ...
      if yy >= startY and yy < endY then
        ...
        lcd.drawNumber(342,  yy, j, SMLSIZE+RIGHT+CUSTOM_COLOR)
      end
      ii=ii+1;
  end
  ...
  -- hud bitmap
  -------------------------------------
  lcd.drawBitmap(utils.getBitmap("hud_240x130"),120,18)
  ...
  if math.abs(alt) > 999 or alt < -99 then
    lcd.drawNumber(359,63,alt,MIDSIZE+CUSTOM_COLOR+RIGHT)
    if status.terrainEnabled == 1 then
      lcd.setColor(CUSTOM_COLOR, utils.colors.white)
      lcd.drawNumber(350,92,homeAlt,CUSTOM_COLOR+RIGHT+MIDSIZE)
    end
  elseif math.abs(alt) >= 10 then
    lcd.setColor(CUSTOM_COLOR, utils.colors.green) --green
    lcd.drawNumber(359,63,alt,DBLSIZE+CUSTOM_COLOR+RIGHT)
    if status.terrainEnabled == 1 then
      lcd.setColor(CUSTOM_COLOR, utils.colors.white)
      lcd.drawNumber(350,92,homeAlt,CUSTOM_COLOR+RIGHT+MIDSIZE)
    end
    ...
    lcd.setColor(CUSTOM_COLOR, utils.colors.green) --green
    lcd.drawNumber(359,63,alt*10,DBLSIZE+PREC1+CUSTOM_COLOR+RIGHT)
    if status.terrainEnabled == 1 then
      lcd.setColor(CUSTOM_COLOR, utils.colors.white)
      lcd.drawNumber(350,92,homeAlt*10,PREC1+CUSTOM_COLOR+RIGHT+MIDSIZE)
    end
  end
  ...
  if status.airspeedEnabled == 1 then
    ...
    lcd.setColor(CUSTOM_COLOR,WHITE)
    lcd.drawText(186,96,"G",CUSTOM_COLOR+SMLSIZE+RIGHT)
  end
  if (math.abs(speed) >= 10) then
    lcd.setColor(CUSTOM_COLOR,utils.colors.green) --green
    lcd.drawNumber(124,63,speed,DBLSIZE+CUSTOM_COLOR)
    if status.airspeedEnabled == 1 then
      lcd.setColor(CUSTOM_COLOR, utils.colors.white)
      lcd.drawNumber(124,92,hSpeed,CUSTOM_COLOR+MIDSIZE)
    end
  else
    lcd.setColor(CUSTOM_COLOR,utils.colors.green) --green
    lcd.drawNumber(124,63,speed*10,DBLSIZE+CUSTOM_COLOR+PREC1)
    if status.airspeedEnabled == 1 then
      lcd.setColor(CUSTOM_COLOR, utils.colors.white)
      lcd.drawNumber(124,92,hSpeed*10,CUSTOM_COLOR+PREC1+MIDSIZE)
    end
  end
  ...
  if conf.enableWIND == true then
    ...
    lcd.setColor(CUSTOM_COLOR, utils.colors.white)
    lcd.drawText(187,129,"W",CUSTOM_COLOR+SMLSIZE+RIGHT)
    lcd.drawNumber(123,124,telemetry.trueWindSpeed*conf.horSpeedMultiplier,PREC1+CUSTOM_COLOR+MIDSIZE)
  end
  ...
  -- compass ribbon
  libs.drawLib.drawCompassRibbon(18,myWidget,220,130,350,25,true)
  -- pitch and roll
