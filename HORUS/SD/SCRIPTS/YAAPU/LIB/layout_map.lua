...
local function drawLineByOriginAndAngle(drawLib,ox,oy,angle,len,style,xmin,xmax,ymin,ymax,color,radio,rev)
...
local function drawMap(myWidget,drawLib,conf,telemetry,status,battery,utils,level)
  ...
  if telemetry.lat ~= nil and telemetry.lon ~= nil then
    ...
    if getTime() - lastPosUpdate > 50 and cycle%2==0 then
      posUpdated = true
    ...
    if getTime() - lastHomePosUpdate > 50 and posUpdated and cycle%2==0 then
    ...
    if myScreenX ~= nil then
      ...
      if status.wpEnabledMode == 1 and status.wpEnabled == 1 and telemetry.wpNumber > 0 then
        ...
        lcd.drawBitmap(utils.getBitmap("maps_box_60x16"),(LCD_W-300)/2+300-62,18+2)
        lcd.drawBitmap(utils.getBitmap("maps_box_60x16"),(LCD_W-300)/2+300-62,18+20)
        lcd.drawBitmap(utils.getBitmap("maps_box_60x16"),(LCD_W-300)/2+300-62,18+40)
        lcd.drawText((LCD_W-300)/2+300-2, 18+2, string.format("#%d",telemetry.wpNumber),SMLSIZE+CUSTOM_COLOR+RIGHT)
        lcd.drawText((LCD_W-300)/2+300-2, 18+20, string.format("%d%s",telemetry.wpDistance * unitScale,unitLabel),SMLSIZE+CUSTOM_COLOR+RIGHT)
        lcd.drawText((LCD_W-300)/2+300-2, 18+40, string.format("%d",telemetry.wpBearing),SMLSIZE+CUSTOM_COLOR+RIGHT)
        -- draw current waypoint info in white
        -- calc new position on odd cycles
        if cycle%2==1 and status.wpLat ~= nil and status.wpLon ~= nil then
          tile_x,tile_y,offset_x,offset_y = coord_to_tiles(conf, status.wpLat, status.wpLon, level)
          wpScreenX,wpScreenY = getScreenCoordinates(conf,minX,minY,tile_x,tile_y,offset_x,offset_y,level)
        end
        if wpScreenX ~= nil then
          local myCode = drawLib.computeOutCode(wpScreenX, wpScreenY, minX+4, minY+4, maxX-4, maxY-4);
          lcd.setColor(CUSTOM_COLOR,utils.colors.white)
          if myCode == 0 then
            ...
            drawLineByOriginAndAngle(drawLib, myScreenX, myScreenY, telemetry.wpBearing-90, 2*300, DOTTED, (LCD_W-300)/2+2, (LCD_W-300)/2+300-2, 18, 18+200, CUSTOM_COLOR, radio, rev)
          end
        end
      end
    end
    ...
    if status.cog ~= nil then
      lcd.drawBitmap(utils.getBitmap("maps_box_60x16"),(LCD_W-300)/2+3,18+41)
      lcd.drawText((LCD_W-300)/2+5,18+40,string.format("cog:%d",status.cog),SMLSIZE+CUSTOM_COLOR)
    end
  end
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
end
...
local function draw(myWidget,drawLib,conf,telemetry,status,battery,alarms,frame,utils,customSensors,leftPanel,centerPanel,rightPanel)
  ...
  drawLib.drawGraph("map_alt", 266, 184, 120, 30, utils.colors.darkyellow, telemetry.homeAlt, false, true, "m")
