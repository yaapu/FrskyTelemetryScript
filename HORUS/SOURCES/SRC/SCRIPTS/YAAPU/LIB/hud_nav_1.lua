...
local function drawVehicle(drawLib,x,y,r,angle,style,xmin,xmax,ymin,ymax,color)
  ...
  --
  drawLib.drawLineWithClippingXY(x1,y1,x2,y2,style,xmin,xmax,ymin,ymax,color,radio,rev)
  drawLib.drawLineWithClippingXY(x1,y1,x3,y3,style,xmin,xmax,ymin,ymax,color,radio,rev)
  drawLib.drawLineWithClippingXY(x2,y2,x4,y4,style,xmin,xmax,ymin,ymax,color,radio,rev)
  drawLib.drawLineWithClippingXY(x3,y3,x4,y4,style,xmin,xmax,ymin,ymax,color,radio,rev)
end

local function updateMyPosition(myWidget,drawLib,conf,telemetry,status,battery,utils)
...
local function drawHud(myWidget,drawLib,conf,telemetry,status,battery,utils)
  ...
  --
  updateMyPosition(myWidget,drawLib,conf,telemetry,status,battery,utils)
  ...
  local myCode = drawLib.computeOutCode(myScreenX, myScreenY, minX+20, minY+20,maxX-20, maxY-20);
  ...
  do
    drawLib.drawLineWithClippingXY(xOffset+v*myTileSize,minY,xOffset+v*myTileSize,maxY,SOLID,minX+1,maxX-1,minY+1,maxY-1,CUSTOM_COLOR,radio,rev)
  end
  ...
  do
    drawLib.drawLineWithClippingXY(minX,yOffset+h*myTileSize,maxX,yOffset+h*myTileSize,SOLID,minX+1,maxX-1,minY+1,maxY-1,CUSTOM_COLOR,radio,rev)
  end

  local homeCode = drawLib.computeOutCode(originX, originY, minX+10, minY+10,maxX-10, maxY-10);
  ...
  if originXclip ~= originX or originYclip ~= originY then
    ...
    drawLib.drawHomeIcon(originXclip-10/2,originYclip-10/2,utils)
  end
  ...
  do
    ...
    if (xx ~= myScreenX or yy ~= myScreenY) and drawLib.computeOutCode(xx, yy, minX+3, minY+3, maxX-3, maxY-3) == 0 then
        lcd.drawFilledRectangle(xx,yy,2,2,CUSTOM_COLOR,0)
    end
  end

  lcd.setColor(CUSTOM_COLOR,0xFFFF)

  drawVehicle(drawLib,myScreenX,myScreenY,20,telemetry.yaw,SOLID,minX,maxX,minY,maxY,CUSTOM_COLOR)
