...
local function drawHud(myWidget,drawLib,conf,telemetry,status,battery,utils)
  ...
  if bit32.band(myCode,1) == 1 then
    ...
    local homeCode = drawLib.computeOutCode(newOriginX, originY, minX+20, minY+20,maxX-20, maxY-20);
  ...
  if bit32.band(myCode,8) == 8 then
    ...
    -- let's check if home would be visible
    local homeCode = drawLib.computeOutCode(originX, newOriginY, minX+20, minY+20,maxX-20, maxY-20);
  ...
  drawLib.drawHomeIcon(originX-20/2,originY-20/2,utils)
