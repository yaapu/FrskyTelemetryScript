--
-- An FRSKY S.Port <passthrough protocol> based Telemetry script for Taranis QX7
--
...
local battVoltPos = {
  ...
  y = 29,
  yV = 29,
  flags = INVERS+PREC1+SMLSIZE,
  flagsV = INVERS+SMLSIZE
}
...
local battCurrPos = {
  ...
  y = 36,
  yA = 36,
  flags = INVERS+SMLSIZE+PREC1,
  flagsA = INVERS+SMLSIZE
}
...
local function drawGPSStatus()
	...
	--
  if radioModel == "x7" then
    lcd.drawLine(xx ,yy + 12,xx+66,yy + 12,SOLID,0)
  else
  ...
  lcd.drawLine(xx + 34,yy+1,xx+34,yy+12,SOLID,0)
	...
	if gpsStatus  > 2 then
		...
		lcd.drawText(xx + 4,yy + 5, strStatus, SMLSIZE)
		lcd.drawNumber(lcd.getLastRightPos() + 1, yy, numSats, MIDSIZE)
		
    lcd.drawText(xx + 39, yy + 5 , "H", SMLSIZE)
		--
		if gpsHdopC > 100 then
			lcd.drawNumber(lcd.getLastRightPos() + 1, yy, gpsHdopC , MIDSIZE+PREC1+flags)
		else
			lcd.drawNumber(lcd.getLastRightPos() + 1, yy, gpsHdopC , MIDSIZE+PREC1+flags)
		end
	else
...
local function drawRoll()
  ...
  -- no roll ==> segments are vertical, offsets are multiples of r2
	...
	drawCroppedLine(rollX + dx - cx,dy + 32 + cy,r,10,DOTTED,hudPos.x,hudPos.x + hudPos.width,yPos,bottomBarPos.y)
	drawCroppedLine(rollX + dx,dy + 32,r,56 + delta,SOLID,hudPos.x,hudPos.x + hudPos.width,yPos,bottomBarPos.y)
	drawCroppedLine(rollX + dx + cx,dy + 32 - cy,r,10,DOTTED,hudPos.x,hudPos.x + hudPos.width,yPos,bottomBarPos.y)
	drawCroppedLine(rollX + dx + ccx,dy + 32 - ccy,r,7,DOTTED,hudPos.x,hudPos.x + hudPos.width,yPos,bottomBarPos.y)
	drawCroppedLine(rollX + dx + cccx,dy + 32 - cccy,r,5,DOTTED,hudPos.x,hudPos.x + hudPos.width,yPos,bottomBarPos.y)
end
...
local function run(event) 
	processTelemetry()
  --
  if event == EVT_ROT_LEFT or event == EVT_ROT_RIGHT or event == EVT_PLUS_FIRST or event == EVT_MINUS_FIRST then
