...
local function drawHud(drawLib,conf,telemetry,status,battery,getMaxValue)
  ...
  if math.abs(hSpeed) > 99 then
    lcd.drawNumber(32+1,33 - 3,hSpeed*0.1,0)
  else
    lcd.drawNumber(32+1,33 - 3,hSpeed,PREC1)
  end
  ...
  if status.timerRunning == 0 then
    if telemetry.statusArmed == 1 then
      lcd.drawText(32 + 64/2 - 15, 30, " ARMED ", SMLSIZE+INVERS)
    else
      lcd.drawText(32 + 64/2 - 23, 30, " DISARMED ", SMLSIZE+INVERS+BLINK)
    end
  end
  -- yaw angle box
