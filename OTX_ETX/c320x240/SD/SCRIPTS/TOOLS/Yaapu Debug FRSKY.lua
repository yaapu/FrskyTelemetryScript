...
local function run(event)
  ...
  do
    ...
    lcd.drawText(20,22+(i*ROW_H),string.format("0x50%02X: rate: %.01fHz, count: %d, ", i, packetStats[0x5000+i].avg, packetStats[0x5000+i].tot),CUSTOM_COLOR)
  end
  lcd.drawText(LCD_W-10, 1,string.format("link rate: %.01fHz", packetStats.link_rate), CUSTOM_COLOR+RIGHT)

  lcd.drawText(1,LCD_H-ROW_H-4,tostring("/LOGS/"..logfilename),CUSTOM_COLOR)
  collectgarbage()
  collectgarbage()
  return 0
end
