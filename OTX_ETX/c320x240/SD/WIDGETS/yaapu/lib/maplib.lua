...
local function google_coord_to_tiles(lat, lng, level)
  ...
  return math.floor(rx/65), math.floor(ry/65), math.floor(rx%65), math.floor(ry%65)
end
...
local function gmapcacther_coord_to_tiles(lat, lon, level)
  ...
  -- return tile_x, tile_y, offset_x, offset_y
  return math.floor(x % world_tiles), math.floor(y % world_tiles), math.floor((x - math.floor(x)) * 65), math.floor((y - math.floor(y)) * 65)
end
...
local function loadAndCenterTiles(tile_x,tile_y,offset_x,offset_y,width,height,level)
    ...
    local margin = 65 * 0.1 -- 10%
    
    if offset_y < margin then
        yy = yy + 1
    elseif offset_y > (65 - margin) then
        yy = yy - 1
    end

    if offset_x < margin then
        xx = xx + 1
    elseif offset_x > (65 - margin) then
        xx = xx - 1
    end
  for x=1,TILES_X
  do
...
local function drawTiles(width,xmin,xmax,ymin,ymax,color,level)
  ...
  do
    ...
    do
      local idx = width*(y-1)+x
      if tiles[idx] ~= nil then
        lcd.drawBitmap(getTileBitmap(tiles[idx]), xmin+(x-1)*65, ymin+(y-1)*65,65)
      end
    end
  end
  if conf.enableMapGrid then
    ...
    for x=1,TILES_X-1
    do
      lcd.drawLine(xmin+x*65,ymin,xmin+x*65,ymax,DOTTED,color)
    end

    for y=1,TILES_Y-1
    do
      lcd.drawLine(xmin,ymin+y*65,xmax,ymin+y*65,DOTTED,color)
    end
  end
  -- draw 50m or 150ft line at max zoom
  lcd.drawBitmap(utils.getBitmap("maps_box_100x16"),xmin+3,ymax-32)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawLine(xmin+5,ymax-20,xmin+5+scaleLen,ymax-20,SOLID,CUSTOM_COLOR)
  lcd.drawText(xmin+5,ymax-34,string.format("%s (%d)", scaleLabel, level),SMLSIZE+CUSTOM_COLOR)
end
...
local function getScreenCoordinates(minX,minY,tile_x,tile_y,offset_x,offset_y,level)
  ...
  if tcache ~= nil then
    if tiles[tcache[1]] ~= nil then
      -- ok it's on screen
      return minX + (tcache[2]-1)*65 + offset_x, minY + (tcache[3]-1)*65 + offset_y
    end
  end
  -- force offscreen up
  return 320/2, -10
...
local function setupMaps(x, y, level, tiles_x, tiles_y)
  ...
  if level ~= lastZoomLevel then
    ...
    if conf.mapProvider == 1 then
      ...
      scaleLen = ((unitScale==1 and 1 or 3)*50*2^(level+2)/tile_dim)*65
    elseif conf.mapProvider == 2 then
      ...
      scaleLen = ((unitScale==1 and 1 or 3)*50*2^(20-level)/tile_dim)*65
...
function mapLib.drawMap(widget, x, y, w, h, level, tiles_x, tiles_y)
  ...
  local maxX = math.min(320, math.min(minX+w, minX+TILES_X*65))

  local minY = math.max(0,MAP_Y)
  local maxY = math.min(240, math.min(minY+h, minY+TILES_Y*65))
  ...
  if telemetry.lat ~= nil and telemetry.lon ~= nil then
    ...
    if getTime() - lastPosUpdate > 50 and drawCycle%2==0 then
      ...
      -- check if offscreen
      local myCode = libs.drawLib.computeOutCode(myScreenX, myScreenY, minX+12, minY+12, maxX-12, maxY-12);
    ...
    if myScreenX ~= nil then
      lcd.setColor(CUSTOM_COLOR,WHITE)
      libs.drawLib.drawRVehicle(myScreenX,myScreenY,12-5,telemetry.yaw,CUSTOM_COLOR)
      lcd.setColor(CUSTOM_COLOR,BLACK)
      libs.drawLib.drawRVehicle(myScreenX,myScreenY,12,telemetry.yaw,CUSTOM_COLOR)
      ...
      if status.wpEnabledMode == 1 and status.wpEnabled == 1 and telemetry.wpNumber > 0 then
        ...
        if wpScreenX ~= nil then
          ...
          if myCode == 0 then
            ...
            libs.drawLib.drawLineByOriginAndAngle(myScreenX, myScreenY, telemetry.wpBearing-90, 2*260, DOTTED, minX, maxX, minY, maxY, CUSTOM_COLOR, false)
          end
        end
      end
    end
    ...
    do
      if p ~= (sampleCount-1)%10 then
        ...
        if tilesXYByPath[posHistory[p][1]] ~= nil then
          ...
          lcd.drawFilledRectangle(minX + (x-1)*65 + posHistory[p][2]-1, minY + (y-1)*65 + posHistory[p][3]-1,3,3,CUSTOM_COLOR)
        end
      end
    end
  end
