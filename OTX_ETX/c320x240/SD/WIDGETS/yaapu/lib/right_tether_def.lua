...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+1] * 0.01 < 10 then
    lcd.drawNumber(x+66+2, y+-4, status.battery[1+1] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+66+2, y+-4, (status.battery[1+1] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end
  local lx = x+67
  lcd.drawText(lx, y+14, "V", flags)
  ...
  if status.battery[1+2] * 0.01 < 10 then
    lcd.drawNumber(x+66+2, y+45, status.battery[1+2] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+66+2, y+45, (status.battery[1+2] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end

  lx = x+67
  lcd.drawText(lx, y+59, "V", flags)
  lcd.drawText(lx, y+45, status.battsource, flags)
  lcd.setColor(CUSTOM_COLOR, lcd.RGB(140, 140, 140))
  lcd.drawText(x, y+-2, "TETH", flags)
  lcd.drawText(x, y+45, "B2", flags)

  -- batt2 capacity bar %
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(255,255, 255))
  lcd.drawFilledRectangle(x+7, y+95,66,18,CUSTOM_COLOR)
  if perc2 > 50 then
  ...
  lcd.drawGauge(x+7, y+95,66,18,perc2,100,CUSTOM_COLOR)
  -- battery 2 percentage
  lcd.setColor(CUSTOM_COLOR,utils.colors.black) -- black
  local strperc2 = string.format("%02d%%",perc2)
  lcd.drawText(x+33, y+92, strperc2, MIDSIZE+CUSTOM_COLOR)

  -- POWER --
  -- power 1
  lcd.setColor(CUSTOM_COLOR,WHITE) -- white
  local power1 = status.battery[4+1]*status.battery[7+1]*0.01
  lcd.drawNumber(x+63,y+21,power1,MIDSIZE+RIGHT+CUSTOM_COLOR)
  lcd.drawText(x+64,y+27,"W",CUSTOM_COLOR)
  -- power 2
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  local power2 = status.battery[4+2]*status.battery[7+2]*0.01
  lcd.drawNumber(x+63,y+72,power2,MIDSIZE+RIGHT+CUSTOM_COLOR)
  lcd.drawText(x+64,y+78,"W",CUSTOM_COLOR)
end
