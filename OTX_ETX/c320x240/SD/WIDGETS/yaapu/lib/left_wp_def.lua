...
function panel.draw(widget, x, y, battId)
  ...
  if conf.rangeFinderMax > 0 then
    ...
    if rng > conf.rangeFinderMax and status.showMinMaxValues == false then
      ...
      lcd.drawFilledRectangle(x+7-65, y+4+4,65,21,CUSTOM_COLOR)
    end
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawText(x+7, y+4, string.format("%.1f",rng*0.01*unitScale), MIDSIZE+flags+CUSTOM_COLOR)
  else
    flags = BLINK
    ...
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawNumber(x+7, y+4, alt*unitScale, MIDSIZE+flags+CUSTOM_COLOR)
  end
  -- LABELS
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  ...
  if dist < 999 then
    lcd.drawText(x+7, y+35, strdist, MIDSIZE+flags+CUSTOM_COLOR)
  else
    lcd.drawText(x+7, y+35, string.format("%0.2f%s", dist*unitLongScale, unitLongLabel), MIDSIZE+flags+CUSTOM_COLOR)
  end
  -- total distance
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  ...
  if status.wpEnabledMode == 1 then
    ...
    -- wp distance
    lcd.drawNumber(x+7, y+80, telemetry.wpDistance * unitScale,MIDSIZE+CUSTOM_COLOR)
    -- LINES
    lcd.setColor(CUSTOM_COLOR,utils.colors.white) --yellow
    -- wp bearing
    libs.drawLib.drawRVehicle(x+66,y+99,9,telemetry.wpOffsetFromCog,CUSTOM_COLOR)
  else
    -- wp number
    lcd.drawText(x+7, y+99, "# ---",SMLSIZE+CUSTOM_COLOR)
    -- wp distance
    lcd.drawText(x+7, y+80, "---",MIDSIZE+CUSTOM_COLOR)
    -- LINES
    lcd.setColor(CUSTOM_COLOR,utils.colors.white) --yellow
    -- wp bearing
    libs.drawLib.drawRVehicle(x+66, y+99, 9, 0, CUSTOM_COLOR)
  end

  if status.showMinMaxValues == true then
    libs.drawLib.drawVArrow(x+3, y+4+4,true,false)
    libs.drawLib.drawVArrow(x+3, y+35+4 ,true,false)
  end
end
