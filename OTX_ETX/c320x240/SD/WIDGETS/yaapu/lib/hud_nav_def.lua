...
function panel.draw(widget)
  ...
  local minX = (320-280)/2
  local maxX = (320-280)/2 + 280
  ...
  if myCode == 1 or myCode == 2 or myCode == 8 or myCode == 4 then
      originX = (320-280)/2+280/2 - zoom*myX;
      originY = 18+134/2 - zoom*myY
  end


  -- round minX to closest tileSize multiple
  local xStart = (320-280)/2 + 280/2 + 10 - (1+math.floor(280/2/tileSize))*tileSize
  ...
  --[[
  lcd.drawText((320-280)/2,18,string.format("yaw:%d",telemetry.yaw),SMLSIZE+CUSTOM_COLOR)
  lcd.drawText((320-280)/2,18+15,string.format("angle:%d",telemetry.homeAngle),SMLSIZE+CUSTOM_COLOR)

  lcd.drawText((320-280)/2,18+30,string.format("myX:%d",myX),SMLSIZE+CUSTOM_COLOR)
  lcd.drawText((320-280)/2,18+60,string.format("myY:%d",myY),SMLSIZE+CUSTOM_COLOR)

  lcd.drawText((320-280)/2,18+80,string.format("myScreenX:%d",myScreenX),SMLSIZE+CUSTOM_COLOR)
  lcd.drawText((320-280)/2,18+100,string.format("myScreenY:%d",myScreenY),SMLSIZE+CUSTOM_COLOR)

  lcd.drawText((320-280)/2,18+80,string.format("myScreenX:%d",myScreenX),SMLSIZE+CUSTOM_COLOR)
  lcd.drawText((320-280)/2,18+100,string.format("myScreenY:%d",myScreenY),SMLSIZE+CUSTOM_COLOR)

  lcd.drawText((320-280)/2,18+120,string.format("avgDistSum:%d",avgDistSum),SMLSIZE+CUSTOM_COLOR)
  --]]
  lcd.drawNumber((320-280)/2,18+140,avgDist,SMLSIZE+CUSTOM_COLOR)

  --[[
  for a=0, math.min(avgDistSampleCount-1,10-1)
  do
