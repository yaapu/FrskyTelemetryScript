...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+1] * 0.01 < 10 then
    lcd.drawNumber(x+56+2, y+-4, status.battery[1+1] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+56+2, y+-4, (status.battery[1+1] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end

  lcd.setColor(CUSTOM_COLOR,lcd.RGB(140, 140, 140))
  lcd.drawText(x+59, y+93, "HYB", SMLSIZE+CUSTOM_COLOR)

  local lx = x+57
  lcd.drawText(lx, y+11, "V", flags)
  lcd.drawText(lx, y+-2, status.battsource, flags)

  lcd.setColor(CUSTOM_COLOR,utils.colors.white) -- white
  -- battery current
  local lowAmp = status.battery[7+1]*0.1 < 10
  libs.drawLib.drawNumberWithDim(x+56,y+118,x+57,y+131,status.battery[7+1]*(lowAmp and 1 or 0.1),"A",DBLSIZE+RIGHT+CUSTOM_COLOR+(lowAmp and PREC1 or 0),0+CUSTOM_COLOR)
  ...
  lcd.drawText(x+40, y+99, strmah, 0+CENTER+CUSTOM_COLOR)
  -- fuel gauge from battery 2
  lcd.setColor(CUSTOM_COLOR,utils.colors.red)
  libs.drawLib.drawGauge(x+13, y+30 , "fuelgauge", 420, 94, 25, 8, status.battery[16+2], 125, CUSTOM_COLOR)
end
