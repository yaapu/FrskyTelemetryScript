...
function panel.draw(widget, x, y, battId)
  ...
  lcd.drawFilledRectangle(x+7, y+50,66,25,CUSTOM_COLOR)
  if perc > 50 then
  ...
  lcd.drawGauge(x+7, y+50,66,25,perc,100,CUSTOM_COLOR)
  -- battery percentage
  lcd.setColor(CUSTOM_COLOR,utils.colors.black) -- black

  local strperc = string.format("%02d%%",perc)
  lcd.drawText(x+26, y+47, strperc, DBLSIZE+CUSTOM_COLOR)
  ...
  if status.battery[1+battId] * 0.01 < 10 then
    lcd.drawNumber(x+67+2, y+9, status.battery[1+battId] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+67+2, y+9, (status.battery[1+battId] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end

  local lx = x+68
  lcd.drawText(lx, y+22, "V", flags)
  -- labels
  ...
  lcd.drawText(x+73, y+78, "IMU TEMP", SMLSIZE+RIGHT+CUSTOM_COLOR)

  -- IMU Temperature
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(x+73, y+86, string.format("%d%s",telemetry.imuTemp, conf.degSymbol), DBLSIZE+RIGHT+CUSTOM_COLOR)

  if status.showMinMaxValues == true then
    drawLib.drawVArrow(x+67+11, y+9 + 8,false,true)
  end
