...
local function drawTelemetryBar()
  ...
  if status.battery[1] * 0.01 < 10 then
    lcd.drawNumber(320-2, 25, status.battery[1] + 0.5, PREC2+MIDSIZE+CUSTOM_COLOR+RIGHT)
  else
    lcd.drawNumber(320-2, 25, (status.battery[1] + 0.5)*0.1, PREC1+MIDSIZE+CUSTOM_COLOR+RIGHT)
  end
  -- aggregate batt %
  local strperc = string.format("%2d", status.battery[16])
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-3, 50, "BATT %", SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(320-3, 60, strperc, MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-2, 121, string.format("%s %s", speedLabel, conf.horSpeedLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(320-2, 131, string.format("%.01f",speed), MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-2, 156, string.format("HOME %s", label), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawNumber(320-2, 166, dist, MIDSIZE+flags+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-3, 85, string.format("%s %s", altLabel, unitLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(320-3, 97, string.format("%.0f",alt), MIDSIZE+CUSTOM_COLOR+RIGHT)
  -- home angle
  lcd.setColor(CUSTOM_COLOR,utils.colors.darkyellow)
  libs.drawLib.drawRVehicle(300,216,16,math.floor(telemetry.homeAngle - telemetry.yaw),CUSTOM_COLOR)
end
...
--]]
      lcd.drawText(PLOT_X+PLOT_W/2,PLOT1_Y-2,status.plotSources[conf.plotSource1][1],CUSTOM_COLOR+SMLSIZE+CENTER)
      lcd.drawText(PLOT_X,PLOT1_Y,string.format("%d", val1Max),CUSTOM_COLOR+SMLSIZE)
      lcd.drawText(PLOT_X,PLOT1_Y+PLOT_H-7,string.format("%d", val1Min),CUSTOM_COLOR+SMLSIZE)
    ...
    if conf.plotSource2 > 1 then
      ...
      lcd.drawText(PLOT_X,PLOT2_Y+PLOT_H-7,string.format("%d", val2Min),CUSTOM_COLOR+SMLSIZE)
