...
local function drawConfigMenuBars()
  ...
  lcd.drawFilledRectangle(0,0, 320, 18, CUSTOM_COLOR)
  lcd.drawRectangle(0, 0, 320, 18, CUSTOM_COLOR)
  lcd.drawFilledRectangle(0,240-18, 320, 18, CUSTOM_COLOR)
  lcd.drawRectangle(0, 240-18, 320, 18, CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(320,3,"Yaapu Telemery 2.2.x dev".." ("..'2964276'..")",CUSTOM_COLOR+SMLSIZE+RIGHT)
  lcd.drawText(0,0,info.name,CUSTOM_COLOR)
  lcd.drawText(0,240-18+1,getConfigFilename(),CUSTOM_COLOR)
  lcd.drawText(320,240-18+1,itemIdx,CUSTOM_COLOR+RIGHT)
end
...
local function drawItem(idx,flags)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  if type(menuItems[idx][4]) == "table" then
    lcd.drawText(200,18 + (idx-menu.offset-1)*17, menuItems[idx][4][menuItems[idx][3]],flags+CUSTOM_COLOR)
  else
    if menuItems[idx][3] == 0 and menuItems[idx][4] >= 0 then
      lcd.drawText(200,18 + (idx-menu.offset-1)*17, "---",flags+CUSTOM_COLOR)
    else
      lcd.drawNumber(200,18 + (idx-menu.offset-1)*17, menuItems[idx][3],flags+menuItems[idx][7]+CUSTOM_COLOR)
      if menuItems[idx][6] ~= nil then
        lcd.drawText(200 + 50,18 + (idx-menu.offset-1)*17, menuItems[idx][6],flags+CUSTOM_COLOR)
      end
    end
  end
end
...
local function drawConfigMenu(event)
  ...
  for m=1+menu.offset,math.min(#menuItems,12+menu.offset) do
    lcd.setColor(CUSTOM_COLOR,WHITE)
    lcd.drawText(2,18 + (m-menu.offset-1)*17, menuItems[m][1],CUSTOM_COLOR)
