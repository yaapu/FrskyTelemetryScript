...
utils.pushMessage = function(severity, msg)
  ...
  if lenLong > 48 then
    ...
    local revIdx = sFind(sRev(longMsg), " ", lenLong - 48)
    local splitPos = revIdx and (lenLong - revIdx) or 48
...
local function drawRssi()
  -- RSSI
  lcd.drawText(213, 0, "RS:", 0+CUSTOM_COLOR)
  lcd.drawText(213 + 20, 0, getRSSI(), 0+CUSTOM_COLOR)
end

local function drawRssiCRSF()
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  -- RSSI
  lcd.drawText(213 - 128, 0, "RTP:", 0+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(213, 0, "RS:", 0+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(213 - 128 + 20, 0, string.format("%d/%d/%d",getValue("RQly"),getValue("TQly"),getValue("TPWR")), 0+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(213 + 22, 0, string.format("%d/%d", telemetry.rssiCRSF, getValue("RFMD")), 0+CUSTOM_COLOR+SMLSIZE)
end
...
local function reset()
  if resetPending then
    ...
    elseif resetPhase == 7 then
      utils.pushMessage(7,"Yaapu Telemetry Widget 2.2.x dev".." ("..'2964276'..")")
...
local function drawMessagesTelemetryBar()
  ...
  lcd.drawText(320-2, 11, string.upper(status.battsource).." V", SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  if status.battery[1] * 0.01 < 10 then
    lcd.drawNumber(320-2, 20, status.battery[1] + 0.5, PREC2+MIDSIZE+CUSTOM_COLOR+RIGHT)
  else
    lcd.drawNumber(320-2, 20, (status.battery[1] + 0.5)*0.1, PREC1+MIDSIZE+CUSTOM_COLOR+RIGHT)
  end

  -- aggregate batt %
  local strperc = string.format("%2d", status.battery[16])
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-3, 42, "BATT %", SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(320-3, 51, strperc, MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-2, 105, string.format("%s %s", speedLabel, conf.horSpeedLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(320-2, 114, string.format("%.01f",speed), MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-2, 136, string.format("HOME %s", label), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawNumber(320-2, 145, dist, MIDSIZE+flags+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(320-3, 74, string.format("%s %s", altLabel, unitLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(320-3, 83, string.format("%.0f",alt), MIDSIZE+CUSTOM_COLOR+RIGHT)

  -- home angle
  lcd.setColor(CUSTOM_COLOR,utils.colors.darkyellow)
  libs.drawLib.drawRVehicle(300,185,16,math.floor(telemetry.homeAngle - telemetry.yaw),CUSTOM_COLOR)
end
...
local function drawMessageScreen()
  ...
  if status.messageAutoScroll == true then
    status.messageOffset = math.max(0, status.messageCount - 13)
  end

  local row = 0
  local offsetStart = status.messageOffset
  local offsetEnd = math.min(status.messageCount-1, status.messageOffset + 13 - 1)

  for i=offsetStart,offsetEnd  do
    lcd.setColor(CUSTOM_COLOR,utils.mavSeverity[status.messages[i % 200][2]][2])
    lcd.drawText(0,14 + 14*row, status.messages[i % 200][1],0+CUSTOM_COLOR)
    row = row+1
  end
  lcd.setColor(CUSTOM_COLOR,utils.colors.bg)
  lcd.drawFilledRectangle(270,11,50,200,CUSTOM_COLOR)
  ...
  local maxPages = tonumber(math.ceil((#status.messages+1)/13))
  local currentPage = 1+tonumber(maxPages - (status.messageCount - status.messageOffset)/13)

  lcd.drawText(320-2, 240-16, string.format("%d/%d",currentPage,maxPages), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,utils.colors.grey)
  lcd.drawLine(0,240-10,320-50,240-10,SOLID,CUSTOM_COLOR)
...
utils.getMessageOffset = function(widget,conf,status,chValue)
  ...
  if conf.screenWheelChannelId > -1 then
    -- SW up
    if chValue < -600 then
      local offset = math.min(status.messageOffset + 13, math.max(0,status.messageCount - 13))
      if offset >= (status.messageCount - 13) then
        status.messageAutoScroll = true
      else
    ...
    if chValue > 600 then
      status.messageAutoScroll = false
      return math.max(0,math.max(status.messageCount - 200,status.messageOffset - 13))
    end
...
local function drawMsgDialog(msg)
  ...
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawFilledRectangle(40,60, 240, 60, CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,utils.colors.bars2)
  lcd.drawFilledRectangle(40+2,60+2, 240-4, 60-4, CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(math.floor(320/2), 73, msg, DBLSIZE+CUSTOM_COLOR+CENTER)
end
...
local function drawFullScreen(widget)
  ...
  if not (resetPending or resetLayoutPending or loadConfigPending) then
    ...
    if drawStatus then
      ...
      libs.drawLib.drawArmStatus(70, 75)
    end
  else
...
local function drawScreen(widget, event, touchState)
    -- horizontal screen 320x240
    if widget.zone.h < 235 then
