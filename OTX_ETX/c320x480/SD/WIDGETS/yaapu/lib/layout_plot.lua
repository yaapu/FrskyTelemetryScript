...
local function drawTelemetryBar()
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(150, 340-3, string.format("%s %s", altLabel, unitLabel), SMLSIZE+CUSTOM_COLOR+0)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(150, 340+7, string.format("%.0f",alt), MIDSIZE+CUSTOM_COLOR+0)
end
...
function layout.draw(widget, customSensors, leftPanel, centerPanel, rightPanel)
  ...
  local nextX = libs.drawLib.drawTerrainStatus(4,40)
  libs.drawLib.drawFenceStatus(nextX,40)
end
