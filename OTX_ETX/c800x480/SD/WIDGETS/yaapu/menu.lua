...
local menuItems = {
  ...
  {"map tile size:", "MAPTSZ", 1, { "default", "256x256" }, { 1, 2 } },
...
local function applyConfigValues(conf)
  ...
  -- screen 4 is for dual battery, we override and force single battery layout
  -- on both left and right panels (it can't be changed)
  conf.centerPanel[4] = conf.centerPanel[1]
  conf.centerPanelFilename[4] = conf.centerPanelFilename[1]
  conf.rightPanel[4] = conf.rightPanel[1]
  conf.rightPanelFilename[4] = "right_def"
  conf.leftPanel[4] = conf.leftPanel[1]
  conf.leftPanelFilename[4] = "right_def"
...
local function drawConfigMenuBars()
  ...
  lcd.drawRectangle(0, 0, LCD_W, 32, CUSTOM_COLOR)
  lcd.drawFilledRectangle(0,LCD_H-32, LCD_W, 32, CUSTOM_COLOR)
  lcd.drawRectangle(0, LCD_H-32, LCD_W, 32, CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(0,0,info.name,CUSTOM_COLOR)
  lcd.drawText(0,LCD_H-32+1,getConfigFilename(),CUSTOM_COLOR)
  lcd.drawText(LCD_W,LCD_H-32+1,itemIdx,CUSTOM_COLOR+RIGHT)
end
...
local function drawItem(idx,flags)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  if type(menuItems[idx][4]) == "table" then
    lcd.drawText(550,33 + (idx-menu.offset-1)*26, menuItems[idx][4][menuItems[idx][3]],flags+CUSTOM_COLOR)
  else
    if menuItems[idx][3] == 0 and menuItems[idx][4] >= 0 then
      lcd.drawText(550,33 + (idx-menu.offset-1)*26, "---",flags+CUSTOM_COLOR)
    else
      lcd.drawNumber(550,33 + (idx-menu.offset-1)*26, menuItems[idx][3],flags+menuItems[idx][7]+CUSTOM_COLOR)
      if menuItems[idx][6] ~= nil then
        lcd.drawText(550 + 50,33 + (idx-menu.offset-1)*26, menuItems[idx][6],flags+CUSTOM_COLOR)
      end
    end
  end
end
...
local function drawConfigMenu(event)
  ...
  elseif not menu.editSelected and (event == EVT_VIRTUAL_NEXT or event == EVT_PLUS_BREAK or event == EVT_ROT_RIGHT) then
    menu.selectedItem = (menu.selectedItem + 1)
    if menu.selectedItem - 16 > menu.offset then
      menu.offset = menu.offset + 1
    end
  end
  --wrap
  ...
  elseif menu.selectedItem  < 1 then
    ...
    menu.offset =  #menuItems - 16
  end
  --
  for m=1+menu.offset,math.min(#menuItems,16+menu.offset) do
    lcd.setColor(CUSTOM_COLOR,WHITE)
    lcd.drawText(2,33 + (m-menu.offset-1)*26, menuItems[m][1],CUSTOM_COLOR)
