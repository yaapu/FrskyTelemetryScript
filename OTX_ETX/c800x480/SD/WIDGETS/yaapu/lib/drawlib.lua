...
function drawLib.drawMinMaxBar(x, y, w, h, color, value, min, max, flags)
  ...
  local xOffset = flags==0 and 17 or 28
  local yOffset = flags==0 and 2 or 7
...
function drawLib.drawBar(name, x, y, w, h, color, value, flags)
  ...
  lcd.setColor(CUSTOM_COLOR, BLACK)
  lcd.drawNumber(x+w-1,y-5,value,CUSTOM_COLOR+flags+RIGHT)
  -- border
  lcd.setColor(CUSTOM_COLOR, BLACK)
...
function drawLib.drawCustomSensors(customSensors, customSensorXY, colorLabel)
  ...
  if customSensorXY.boxY ~= nil then
    ...
    lcd.drawFilledRectangle(0,customSensorXY.boxY,LCD_W,58,CUSTOM_COLOR)
    lcd.setColor(CUSTOM_COLOR,utils.colors.grey)
    lcd.drawLine(1,customSensorXY.boxY+58,LCD_W-2,customSensorXY.boxY+58,SOLID,CUSTOM_COLOR)
  end
