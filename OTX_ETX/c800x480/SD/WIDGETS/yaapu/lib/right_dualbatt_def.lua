...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+1] * 0.01 < 10 then
    lcd.drawNumber(x+100+2, y+-4, status.battery[1+1] + 0.5, PREC2+MIDSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+100+2, y+-4, (status.battery[1+1] + 0.5)*0.1, PREC1+MIDSIZE+RIGHT+flags)
  end
  local lx = x+101
  ...
  if status.battery[1+2] * 0.01 < 10 then
    lcd.drawNumber(x+100+2, y+51, status.battery[1+2] + 0.5, PREC2+MIDSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+100+2, y+51, (status.battery[1+2] + 0.5)*0.1, PREC1+MIDSIZE+RIGHT+flags)
  end
