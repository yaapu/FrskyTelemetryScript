...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+1] * 0.01 < 10 then
    lcd.drawNumber(x+167+2, y+-7, status.battery[1+1] + 0.5, PREC2+MIDSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+167+2, y+-7, (status.battery[1+1] + 0.5)*0.1, PREC1+MIDSIZE+RIGHT+flags)
  end
  local lx = x+168
  ...
  if status.battery[1+2] * 0.01 < 10 then
    lcd.drawNumber(x+167+2, y+90, status.battery[1+2] + 0.5, PREC2+MIDSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+167+2, y+90, (status.battery[1+2] + 0.5)*0.1, PREC1+MIDSIZE+RIGHT+flags)
  end

  lx = x+168
  lcd.drawText(lx, y+118, "V", flags)
  ...
  lcd.drawFilledRectangle(x+17, y+190,167,35,CUSTOM_COLOR)
  if perc2 > 50 then
  ...
  lcd.drawGauge(x+17, y+190,167,35,perc2,100,CUSTOM_COLOR)
  -- battery 2 percentage
  lcd.setColor(CUSTOM_COLOR,utils.colors.black) -- black
  local strperc2 = string.format("%02d%%",perc2)
  lcd.drawText(x+83, y+183, strperc2, MIDSIZE+CUSTOM_COLOR)
  ...
  lcd.drawNumber(x+158,y+144,power2,MIDSIZE+RIGHT+CUSTOM_COLOR)
