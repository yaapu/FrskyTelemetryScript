...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+1] * 0.01 < 10 then
    lcd.drawNumber(x+166+2, y+0, status.battery[1+1] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+166+2, y+0, (status.battery[1+1] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end
  local lx = x+168
  lcd.drawText(lx, y+35, "V", flags)
  ...
  if status.battery[1+2] * 0.01 < 10 then
    lcd.drawNumber(x+166+2, y+97, status.battery[1+2] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+166+2, y+97, (status.battery[1+2] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end

  lx = x+168
  lcd.drawText(lx, y+125, "V", flags)
  lcd.drawText(lx, y+97, status.battsource, flags)
  lcd.setColor(CUSTOM_COLOR, lcd.RGB(140, 140, 140))
  lcd.drawText(x, y+-3, "TETH", flags)
  lcd.drawText(x, y+97, "B2", flags)

  -- batt2 capacity bar %
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(255,255, 255))
  lcd.drawFilledRectangle(x+17, y+190,166,35,CUSTOM_COLOR)
  if perc2 > 50 then
  ...
  lcd.drawGauge(x+17, y+190,166,35,perc2,100,CUSTOM_COLOR)
  -- battery 2 percentage
  lcd.setColor(CUSTOM_COLOR,utils.colors.black) -- black
  local strperc2 = string.format("%02d%%",perc2)
  lcd.drawText(x+83, y+187, strperc2, MIDSIZE+CUSTOM_COLOR)

  -- POWER --
  -- power 1
  lcd.setColor(CUSTOM_COLOR,WHITE) -- white
  local power1 = status.battery[4+1]*status.battery[7+1]*0.01
  lcd.drawNumber(x+158,y+49,power1,MIDSIZE+RIGHT+CUSTOM_COLOR)
  lcd.drawText(x+161,y+62,"W",CUSTOM_COLOR)
  -- power 2
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  local power2 = status.battery[4+2]*status.battery[7+2]*0.01
  lcd.drawNumber(x+158,y+148,power2,MIDSIZE+RIGHT+CUSTOM_COLOR)
  lcd.drawText(x+161,y+161,"W",CUSTOM_COLOR)
end
