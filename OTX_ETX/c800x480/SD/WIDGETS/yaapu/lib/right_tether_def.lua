...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+1] * 0.01 < 10 then
    lcd.drawNumber(x+168+2, y+-7, status.battery[1+1] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+168+2, y+-7, (status.battery[1+1] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end
  local lx = x+168
  ...
  if status.battery[1+2] * 0.01 < 10 then
    lcd.drawNumber(x+168+2, y+90, status.battery[1+2] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+168+2, y+90, (status.battery[1+2] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end
