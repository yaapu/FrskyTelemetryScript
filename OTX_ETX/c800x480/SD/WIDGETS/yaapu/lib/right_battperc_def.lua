...
function panel.draw(widget, x, y, battId)
  ...
  lcd.drawFilledRectangle(x+17, y+100,166,49,CUSTOM_COLOR)
  if perc > 50 then
  ...
  lcd.drawGauge(x+17, y+100,166,49,perc,100,CUSTOM_COLOR)
  -- battery percentage
  lcd.setColor(CUSTOM_COLOR,utils.colors.black) -- black

  local strperc = string.format("%02d%%",perc)
  lcd.drawText(x+66, y+98, strperc, DBLSIZE+CUSTOM_COLOR)
  ...
  local lx = x+178
  lcd.drawText(lx, y+50, "V", flags)
  -- labels
  lcd.setColor(CUSTOM_COLOR,lcd.RGB(140, 140, 140))
  lcd.drawText(x+2,y+0,battId == 0 and "B1+B2" or (battId == 1 and "B1" or "B2"),SMLSIZE+CUSTOM_COLOR)
  lcd.drawText(x+183, y+157, "IMU TEMP", SMLSIZE+RIGHT+CUSTOM_COLOR)

  -- IMU Temperature
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(x+183, y+172, string.format("%d%s",telemetry.imuTemp, conf.degSymbol), DBLSIZE+RIGHT+CUSTOM_COLOR)

  if status.showMinMaxValues == true then
    drawLib.drawVArrow(x+177+11, y+22 + 8,false,true)
  end
