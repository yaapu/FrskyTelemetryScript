...
function layout.draw(widget, customSensors, leftPanel, centerPanel, rightPanel)
  ...
  if status.batt2sources.fc or status.batt2sources.vs then
    if status.showDualBattery == false then
      rightPanel[status.currentScreen].draw(widget, 600, 28, 0)
      leftPanel[status.currentScreen].draw(widget, 0, 28, 0)
      battIdForPower = 0
    else
      rightPanel[status.currentScreen].draw(widget, 600, 28, 1)
      leftPanel[status.currentScreen].draw(widget, 0, 28, 2)
    end
  else
    rightPanel[status.currentScreen].draw(widget, 600, 28, 1)
    leftPanel[status.currentScreen].draw(widget, 0, 28, 0)
  ...
  if conf.enableRPM == 2 or conf.enableRPM == 3 then
    lcd.drawText(17, 275, "RPM 1", SMLSIZE+CUSTOM_COLOR)
    libs.drawLib.drawBar("rpm1", 17, 275 + 22, 150, 38, utils.colors.darkyellow, math.abs(telemetry.rpm1), MIDSIZE)
  end

  lcd.setColor(CUSTOM_COLOR,colorLabel)
  if conf.enableRPM == 3 then
    lcd.drawText(192, 275, "RPM 2", SMLSIZE+CUSTOM_COLOR+0)
    libs.drawLib.drawBar("rpm2", 192, 275 + 22, 150, 38, utils.colors.darkyellow, math.abs(telemetry.rpm2), MIDSIZE)
  end

  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(525, 275, "THR %", SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawNumber(525,290,telemetry.throttle,MIDSIZE+RIGHT+CUSTOM_COLOR)

  if status.hideEfficiency == 0 then
    lcd.setColor(CUSTOM_COLOR,colorLabel)
    lcd.drawText(658, 275, "EFF mAh", SMLSIZE+CUSTOM_COLOR+RIGHT)
    ...
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawNumber(658,290+(eff > 9999 and 7 or 0),eff,(eff > 9999 and 0 or MIDSIZE)+RIGHT+CUSTOM_COLOR)
  end
  ...
  if status.hidePower == 0 then
    ...
    lcd.drawText(796, 275, string.format("PWR %s",powerUnit), SMLSIZE+CUSTOM_COLOR+RIGHT)
    lcd.setColor(CUSTOM_COLOR,utils.colors.white)
    lcd.drawNumber(796,290,power*(power > 999 and 0.1 or 1),MIDSIZE+RIGHT+CUSTOM_COLOR+flags)
  end
  ...
  local nextX = libs.drawLib.drawTerrainStatus(203, 60)
