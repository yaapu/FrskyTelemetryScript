...
function panel.draw(widget, x, y, battId)
  ...
  if status.battery[1+battId] * 0.01 < 10 then
    lcd.drawNumber(x+177+2, y+22, status.battery[1+battId] + 0.5, PREC2+DBLSIZE+RIGHT+flags)
  else
    lcd.drawNumber(x+177+2, y+22, (status.battery[1+battId] + 0.5)*0.1, PREC1+DBLSIZE+RIGHT+flags)
  end

  local lx = x+178
  lcd.drawText(lx, y+50, "V", flags+SMLSIZE)
  lcd.setColor(CUSTOM_COLOR,utils.colors.white) -- white
  -- battery voltage
  libs.drawLib.drawNumberWithDim(x+70,y+120,x+70, y+130, status.battery[4+battId],"V",RIGHT+PREC1+CUSTOM_COLOR,SMLSIZE+CUSTOM_COLOR)
  -- battery current
  local lowAmp = status.battery[7+battId]*0.1 < 10
  libs.drawLib.drawNumberWithDim(x+180,y+97,x+180,y+130,status.battery[7+battId]*(lowAmp and 1 or 0.1),"A",DBLSIZE+RIGHT+CUSTOM_COLOR+(lowAmp and PREC1 or 0),SMLSIZE+CUSTOM_COLOR)
  ...
  lcd.drawText(x+2, y+0, battLabel, SMLSIZE+CUSTOM_COLOR)

  if status.showMinMaxValues == true then
    libs.drawLib.drawVArrow(x+177+11, y+22 + 8,false,true)
    libs.drawLib.drawVArrow(x+70+11, y+120 + 3, false,true)
    libs.drawLib.drawVArrow(x+180+11, y+97 + 10,true,false)
  end
  --]]
  lcd.setColor(CUSTOM_COLOR, lcd.RGB(140, 140, 140))
  lcd.drawText(x+183, y+0, string.format("%s CELL",string.upper(status.battsource)), SMLSIZE+RIGHT+CUSTOM_COLOR)
