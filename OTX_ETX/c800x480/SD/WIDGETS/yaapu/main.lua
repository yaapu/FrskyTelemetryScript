...
-- local cache
local gTime = getTime
local pFile = playFile
local pHaptic = playHaptic
local t_number = tonumber
...
utils.playSoundByFlightMode = function(flightMode)
  ...
  if cf.enableHaptic then
    pHaptic(15, 0)
  end
  
  if cf.disableAllSounds then
    return
  end

  local cFT = st.currentFrameType
  local fModes = cFT and cFT.flightModes
  
  if fModes then
    local modeFile = fModes[flightMode]
    if modeFile then
      ut.lcdBacklightOn()
      
      local fType = st.frameTypes[telemetry.frameType]
      local suffix = (fType == "r" or fType == "b") and "_r.wav" or ".wav"
      
      -- rover sound files differ because they lack "flight" word
      pFile(soundFileBasePath .. "/" .. cf.language .. "/" .. modeFile .. suffix)
    end
  end
end
...
local function updateHash(c)
  ...
  local hIdx = st.hashByteIndex
  local pHashes = ut.prefixHashes

  -- check if we need to process this prefix and if so
  -- check if we have a prefix hash
  if hIdx >= pHashes.maxLength and st.prefixHash ~= nil then
    return
  end

  local ha = (st.hash_a + c) % 4095
  local hb = (ha + st.hash_b) % 4095
  local hSum = hb * 4096 + ha
  ...
  hIdx = hIdx + 1
  st.hashByteIndex = hIdx

  local pData = pHashes[hSum]
  if pData ~= nil then
    st.prefixHash = hSum
    st.parsePrefixHash = pData[1]
  end
end
...
local function playHash()
  -- Cache tables for faster access
  local st = status
  local ut = utils
  local pHashes = ut.prefixHashes
  
  -- Note: we try to play hash sound files without checking for existence
  local currentHash = st.hash
  local pHash = st.prefixHash

  if pHash == nil then
    -- play full hash and return
    ut.playSound(currentHash, true)
    return
  end
  ...
  -- Localize prefix config to avoid repeated deep lookups
  local pConfig = pHashes[pHash]

  if st.parsePrefixHash == false then
    -- 1) no regex defined
    ut.playSound(pHash, true)
    return
  end

  -- 2) regex defined, check if we get a match
  local g1, g2, g3 = s_match(st.msgBuffer, pConfig[3])
  ...
  -- Map groups using a local helper to handle dynamic indexing without a new table
  local function getGroup(idx)
    if idx == 1 then return g1 elseif idx == 2 then return g2 elseif idx == 3 then return g3 end
    return nil
  end

  local param = getGroup(pConfig.paramGroup)
  local suffix = getGroup(pConfig.suffixGroup)
  local extra = getGroup(pConfig.extraGroup)

  local prefix = pConfig[2]
  local extraFileToPlay = prefix

  ut.playSound(pHash, true)

  if param ~= nil then
    playNumber(t_number(param), 0)
  ...
  if suffix ~= nil then
    ut.playSound(suffix, false)
  end

  -- check if we need to play an extra file
  if extraFileToPlay ~= nil then
    ut.playSound(extraFileToPlay, false)
  end

  -- play an additional file only if a prefix is defined and [extra] is mapped to a file in the extraMap table
  if extra ~= nil and prefix ~= nil then
    local mappedExtra = pHashes.extraMap[extra]
    if mappedExtra ~= nil then
      ut.playSound(prefix .. mappedExtra, false)
    end
  end
end
...
local function formatMessage(severity,msg)
  ...
  local fTime = st.flightTime
  local minutes = m_floor(fTime / 60)
  local seconds = fTime % 60
  
  local sevPrefix = ut.mavSeverity[severity][1]

  if st.lastMessageCount > 1 then
    -- roll [0,1800] ==> [-180,180] 
    return s_format("%02d:%02d %s (x%d) %s", minutes, seconds, sevPrefix, st.lastMessageCount, msg)
  else
    return s_format("%02d:%02d %s %s", minutes, seconds, sevPrefix, msg)
  end
end
...
utils.pushMessage = function(severity, msg)
  ...
  if now - lastMsgTime > 50 then
    local silence = cf.disableMsgBeep == 3 or (severity >= 5 and cf.disableMsgBeep == 2)
    if not silence then
      local wav = "../" .. ut.mavSeverity[severity][1]
      ut.playSound(wav, true)
    end
    lastMsgTime = now
  end

  if msg == st.lastMessage then
    st.lastMessageCount = st.lastMessageCount + 1
  else
    st.lastMessageCount = 1
    st.messageCount = st.messageCount + 1
    st.messageRow = st.messageRow + 1
  end

  local longMsg = formatMessage(severity, msg)
  local lenLong = sLen(longMsg)
  local rowIdx = (st.messageRow - 1) % 200

  if lenLong > 65 then
    -- search for the first blank before max length
    local revIdx = sFind(sRev(longMsg), " ", lenLong - 65)
    local splitPos = revIdx and (lenLong - revIdx) or 65
    
    if msg == st.lastMessage then
      st.messageRow = st.messageRow - 1
      rowIdx = (st.messageRow - 1) % 200
    end

    -- minimize Garbage Collection by reusage
    if not st.messages[rowIdx] then st.messages[rowIdx] = {} end
    st.messages[rowIdx][1] = sSub(longMsg, 1, splitPos)
    st.messages[rowIdx][2] = severity
    
    st.messageRow = st.messageRow + 1
    rowIdx = (st.messageRow - 1) % 200
    
    if not st.messages[rowIdx] then st.messages[rowIdx] = {} end
    st.messages[rowIdx][1] = "    " .. sSub(longMsg, splitPos + 1, 60)
    st.messages[rowIdx][2] = severity
  else
    if not st.messages[rowIdx] then st.messages[rowIdx] = {} end
    st.messages[rowIdx][1] = longMsg
    st.messages[rowIdx][2] = severity
  end

  st.lastMessage = msg
  st.lastMessageSeverity = severity
...
utils.haversine = function(lat1, lon1, lat2, lon2)
    ...
    local dlat_2 = (lat2 - lat1) / 2
    local dlon_2 = (lon2 - lon1) / 2

    local s_lat = sin(dlat_2)
    local s_lon = sin(dlon_2)
    
    local a = s_lat * s_lat + cos(lat1) * cos(lat2) * s_lon * s_lon
...
utils.getAngleFromLatLon = function(lat1, lon1, lat2, lon2)
    ...
    return (a * DEG + 360) % 360
end

local DEG_180 = 180 / math.pi
local INV_R = 1 / 6371000 -- skip a division
...
utils.getLatLonFromAngleAndDistance = function(telemetry, angle, distance)
    ...
    local la1 = lat * PI_180
    local lo1 = lon * PI_180
    local Ad = distance * INV_R
    local theta = angle * PI_180 -- θ in radiants
    ...
    -- lat
    local sinLa2 = sinLa1 * cosAd + cosLa1 * sinAd * cosTheta
    local la2 = math.asin(sinLa2)
    
    -- lon
    local y = sinTheta * sinAd * cosLa1
    local x = cosAd - sinLa1 * sinLa2
    local lo2 = lo1 + math.atan2(y, x)

    return la2 * DEG_180, lo2 * DEG_180
end

utils.decToDMS = function(dec, lat)
    local absDec = math.abs(dec)
    local D = math.floor(absDec)
    local M = (absDec - D) * 60
    
    local suffix
    if lat then
        suffix = (dec >= 0) and "E" or "W"
    else
        suffix = (dec >= 0) and "N" or "S"
    end

    return D .. conf.degSymbol .. string.format("%05.2f", M) .. suffix
end
...
utils.decToDMSFull = function(dec, lat)
    ...
    local mFull = (absDec - d) * 60
    local m = math.floor(mFull)
    local s = (mFull - m) * 60
    ...
    return d .. conf.degSymbol .. m .. "'" .. string.format("%04.1f", s) .. suffix
end
...
utils.updateTotalDist = function()
  -- Cache global tables for faster access
  local st = status
  local tel = telemetry
  local ut = utils
  ...
  -- Early exit if not armed
  if tel.armingStatus == 0 then
    st.lastUpdateTotDist = now
    return
  end
  
  -- Localizing frequently used coordinates
  local currentLat = tel.lat
  local currentLon = tel.lon

  if currentLat ~= nil and currentLon ~= nil then
    -- Cache previous travel coordinates
    local tLat = st.travelLat
    local tLon = st.travelLon

    if tLat == nil or tLon == nil then
      st.travelLat = currentLat
      st.travelLon = currentLon
      st.lastTravelLatLonSampleTime = now
      return -- Exit early since we just initialized
    end

    -- Calculate time delta using local variable
    if now - st.lastTravelLatLonSampleTime > 50 then
      -- Call haversine using local coordinate values
      local travelDist = ut.haversine(currentLat, currentLon, tLat, tLon)
      
      -- discard sampling errors
      if travelDist < 10000 then
        tel.totalDist = tel.totalDist + travelDist
      end
      
      -- Update status with current telemetry data
      st.travelLat = currentLat
      st.travelLon = currentLon
      st.lastTravelLatLonSampleTime = now
    end
...
local function processTelemetry(appId,value,now)
  -- local global tables cache
  local t = telemetry
  local s = status
  local c = conf

  -- RDY sensor: track link freshness, a passthrough frame was just decoded
  s.lastPassthroughTime = now

  if appId == 0x5006 then -- ROLLPITCH
    -- roll [0,1800] ==> [-180,180]
    t.roll = (m_min(b_extract(value,0,11),1800) - 900) * 0.2
    -- pitch [0,900] ==> [-90,90]
    t.pitch = (m_min(b_extract(value,11,10),900) - 450) * 0.2
    -- number encoded on 11 bits: 10 bits for digits + 1 for 10^power
    t.range = b_extract(value,22,10) * (POW10[b_extract(value,21,1)]) -- cm
  elseif appId == 0x5005 then -- VELANDYAW
    t.vSpeed = b_extract(value,1,7) * (POW10[b_extract(value,0,1)]) * (b_extract(value,8,1) == 1 and -1 or 1)-- dm/s
    t.yaw = b_extract(value,17,11) * 0.2
    -- once detected it's sticky
    if b_extract(value,28,1) == 1 then
      t.airspeed = b_extract(value,10,7) * (POW10[b_extract(value,9,1)]) -- dm/s
    else
      t.hSpeed = b_extract(value,10,7) * (POW10[b_extract(value,9,1)]) -- dm/s
    end
    if s.airspeedEnabled == 0 then
      s.airspeedEnabled = b_extract(value,28,1)
    end
  elseif appId == 0x5001 then -- AP STATUS
    t.flightMode = b_extract(value,0,5)
    t.simpleMode = b_extract(value,5,2)
    t.landComplete = b_extract(value,7,1)
    t.statusArmed = b_extract(value,8,1)
    t.battFailsafe = b_extract(value,9,1)
    t.ekfFailsafe = b_extract(value,10,2)
    t.failsafe = b_extract(value,12,1)
    t.fencePresent = b_extract(value,13,1)
    t.fenceBreached = t.fencePresent == 1 and b_extract(value,14,1) or 0 -- ignore if fence is disabled
    t.throttle = m_floor(0.5 + (b_extract(value,19,6) * (b_extract(value,25,1) == 1 and -1 or 1) * 1.58)) -- signed throttle [-63,63] -> [-100,100]
    -- IMU temperature: 0 means temp =< 19°, 63 means temp => 82°
    t.imuTemp = b_extract(value,26,6) + 19 -- C°
  elseif appId == 0x5002 then -- GPS STATUS
    t.numSats = b_extract(value,0,4)
    ...
    t.gpsStatus = b_extract(value,4,2) + b_extract(value,14,2)
    t.gpsHdopC = b_extract(value,7,7) * (POW10[b_extract(value,6,1)]) -- dm
    t.gpsAlt = b_extract(value,24,7) * (POW10[b_extract(value,22,2)]) * (b_extract(value,31,1) == 1 and -1 or 1)-- dm
  elseif appId == 0x5003 then -- BATT
    telemetry.batt1volt = b_extract(value,0,9)
    ...
    telemetry.batt1current = b_extract(value,10,7) * (POW10[b_extract(value,9,1)])
    telemetry.batt1mah = b_extract(value,17,15)
  elseif appId == 0x5008 then -- BATT2
    telemetry.batt2volt = b_extract(value,0,9)
    -- telemetry max is 51.1V, 51.2 is reported as 0.0, 52.3 is 0.1...60 is 88
    -- if >= 12S and V > 51.1 ==> Vreal = 51.2 + telemetry.batt1volt
    ...
    telemetry.batt2current = b_extract(value,10,7) * (POW10[bit32.extract(value,9,1)])
    telemetry.batt2mah = b_extract(value,17,15)
  elseif appId == 0x5004 then -- HOME
    t.homeDist = b_extract(value,2,10) * (POW10[b_extract(value,0,2)])
    t.homeAlt = b_extract(value,14,10) * (POW10[b_extract(value,12,2)]) * 0.1 * (b_extract(value,24,1) == 1 and -1 or 1)
    t.homeAngle = b_extract(value, 25,  7) * 3
  elseif appId == 0x5000 then -- MESSAGES
    if value ~= s.lastMsgValue then
      s.lastMsgValue = value
      local c_char
      local msgEnd = false
      local chunk = {}
      for i=3,0,-1
      do
        c_char = b_extract(value,i*8,7)
        if c_char ~= 0 then
          --status.msgBuffer = status.msgBuffer .. string.char(c)
          chunk[#chunk + 1] = s_char(c_char)
          updateHash(c_char)
        else
          msgEnd = true
          break
      ...
      s.msgBuffer = (s.msgBuffer or "")..t_concat(chunk)
      if msgEnd then
        local severity = (b_extract(value,7,1) * 1) + (b_extract(value,15,1) * 2) + (b_extract(value,23,1) * 4)
        checkReadyToArmMessage(s.msgBuffer)
  ...
  elseif appId == 0x5007 then -- PARAMS
    t.paramId = b_extract(value,24,4)
    t.paramValue = b_extract(value,0,24)
    if t.paramId == 1 then
      t.frameType = t.paramValue
    elseif t.paramId == 4 then
      t.batt1Capacity = t.paramValue
    elseif t.paramId == 5 then
      t.batt2Capacity = t.paramValue
    elseif t.paramId == 6 then
      t.wpCommands = t.paramValue
    end
  elseif appId == 0x5009 then -- WAYPOINTS @1Hz
    t.wpNumber = b_extract(value,0,10) -- wp index
    t.wpDistance = b_extract(value,12,10) * (POW10[b_extract(value,10,2)]) -- meters
    t.wpXTError = b_extract(value,23,4) * (POW10[b_extract(value,22,1)]) * (b_extract(value,27,1) == 1 and -1 or 1)-- meters
    t.wpOffsetFromCog = b_extract(value,29,3) -- offset from cog with 45° resolution
  elseif appId == 0x500A then -- RPM 1 and 2
    -- rpm1 and rpm2 are int16_t
    local rpm1_raw = b_extract(value,0,16)
    local rpm2_raw = b_extract(value,16,16)
    t.rpm1 = 10*(b_extract(value,15,1) == 0 and rpm1_raw or -1*(1+b_band(0x0000FFFF,b_bnot(rpm1_raw)))) -- 2 complement if negative
    t.rpm2 = 10*(b_extract(value,31,1) == 0 and rpm2_raw or -1*(1+b_band(0x0000FFFF,b_bnot(rpm2_raw)))) -- 2 complement if negative
  elseif appId == 0x500B then -- TERRAIN
    t.heightAboveTerrain = b_extract(value,2,10) * (POW10[b_extract(value,0,2)]) * 0.1 * (b_extract(value,12,1) == 1 and -1 or 1) -- dm to meters
    t.terrainUnhealthy = b_extract(value,13,1)
    s.terrainLastData = now
    s.terrainEnabled = 1
  elseif appId == 0x500C then -- WIND
    t.trueWindSpeed = b_extract(value,8,7) * (POW10[b_extract(value,7,1)]) -- dm/s
    t.trueWindAngle = b_extract(value, 0, 7) * 3 -- degrees
    t.apparentWindSpeed = b_extract(value,23,7) * (POW10[b_extract(value,22,1)]) -- dm/s
    t.apparentWindAngle = b_extract(value, 16, 6) * (b_extract(value,15,1) == 1 and -1 or 1) * 3 -- degrees
  elseif appId == 0x500D then -- WAYPOINTS @1Hz
    t.wpNumber = b_extract(value,0,11) -- wp index
    t.wpDistance = b_extract(value,13,10) * (POW10[b_extract(value,11,2)]) -- meters
    t.wpBearing = b_extract(value, 23,  7) * 3
    if s.cog ~= nil then
      t.wpOffsetFromCog = wrap360(t.wpBearing - s.cog)
    end
    s.wpEnabled = 1
  elseif appId == 0x50F2 then -- VFR
    t.airspeed = b_extract(value,1,7) * (POW10[b_extract(value,0,1)]) -- dm/s
    t.throttle = b_extract(value,8,7) -- unsigned throttle
    t.baroAlt = b_extract(value,17,10) * (POW10[b_extract(value,15,2)]) * 0.1 * (b_extract(value,27,1) == 1 and -1 or 1)
    s.airspeedEnabled = 1
  end
end
...
local function drawRssi()
  ...
  lcd.drawText(538 + 40, 0, getRSSI(), 0+CUSTOM_COLOR)
end

local function drawRssiCRSF()
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  -- RSSI
  lcd.drawText(538 - 240, 0, "RTP:", 0+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(538, 0, "RS:", 0+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(538 - 240 + 40, 0, string.format("%d/%d/%d",getValue("RQly"),getValue("TQly"),getValue("TPWR")), 0+CUSTOM_COLOR+SMLSIZE)
  lcd.drawText(538 + 35, 0, string.format("%d/%d", telemetry.rssiCRSF, getValue("RFMD")), 0+CUSTOM_COLOR+SMLSIZE)
end
...
local function reset()
  if resetPending then
    ...
    elseif resetPhase == 7 then
      utils.pushMessage(7,"Yaapu Telemetry Widget 2.2.x dev".." ("..'33fb4a0'..")")
...
local function drawMessagesTelemetryBar()
  ...
  lcd.drawText(LCD_W-2, 30, string.upper(status.battsource).." V", SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  if status.battery[1] * 0.01 < 10 then
    lcd.drawNumber(LCD_W-2, 47, status.battery[1] + 0.5, PREC2+MIDSIZE+CUSTOM_COLOR+RIGHT)
  else
    lcd.drawNumber(LCD_W-2, 47, (status.battery[1] + 0.5)*0.1, PREC1+MIDSIZE+CUSTOM_COLOR+RIGHT)
  end

  -- aggregate batt %
  local strperc = string.format("%2d", status.battery[16])
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-7, 101, "BATT %", SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(LCD_W-7, 118, strperc, MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-2, 242, string.format("%s %s", speedLabel, conf.horSpeedLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(LCD_W-2, 259, string.format("%.01f",speed), MIDSIZE+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-2, 313, string.format("HOME %s", label), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawNumber(LCD_W-2, 330, dist, MIDSIZE+flags+CUSTOM_COLOR+RIGHT)
  ...
  lcd.setColor(CUSTOM_COLOR,colorLabel)
  lcd.drawText(LCD_W-7, 171, string.format("%s %s", altLabel, unitLabel), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawText(LCD_W-7, 193, string.format("%.0f",alt), MIDSIZE+CUSTOM_COLOR+RIGHT)

  -- home angle
  lcd.setColor(CUSTOM_COLOR,utils.colors.darkyellow)
  libs.drawLib.drawRVehicle(740,431,32,math.floor(telemetry.homeAngle - telemetry.yaw),CUSTOM_COLOR)
end
...
local function drawMessageScreen()
  ...
  if status.messageAutoScroll == true then
    status.messageOffset = math.max(0, status.messageCount - 21)
  end

  local row = 0
  local offsetStart = status.messageOffset
  local offsetEnd = math.min(status.messageCount-1, status.messageOffset + 21 - 1)

  for i=offsetStart,offsetEnd  do
    lcd.setColor(CUSTOM_COLOR,utils.mavSeverity[status.messages[i % 200][2]][2])
    lcd.drawText(0,28 + 20*row, status.messages[i % 200][1],0+CUSTOM_COLOR)
    row = row+1
  end
  lcd.setColor(CUSTOM_COLOR,utils.colors.bg)
  lcd.drawFilledRectangle(675,28,125,LCD_H,CUSTOM_COLOR)
  ...
  local maxPages = tonumber(math.ceil((#status.messages+1)/21))
  local currentPage = 1+tonumber(maxPages - (status.messageCount - status.messageOffset)/21)

  lcd.drawText(LCD_W-2, LCD_H-20, string.format("%d/%d",currentPage,maxPages), SMLSIZE+CUSTOM_COLOR+RIGHT)
  lcd.setColor(CUSTOM_COLOR,utils.colors.grey)
  lcd.drawLine(0,LCD_H-28,LCD_W-125,LCD_H-28,SOLID,CUSTOM_COLOR)

  lcd.setColor(CUSTOM_COLOR,WHITE)
  if status.strFlightMode ~= nil then
    lcd.drawText(0,LCD_H-28,status.strFlightMode,CUSTOM_COLOR)
  end
  lcd.drawTimer(LCD_W-125, LCD_H-28, model.getTimer(2).value, CUSTOM_COLOR+RIGHT)
end
...
local function crossfirePop()
    ...
    if (command == 0x80 or command == 0x7F) and data ~= nil then
      ...
      elseif #data > 4 and data[1] == 0xF1 then
        ...
        checkReadyToArmMessage(status.msgBuffer)
...
utils.getMessageOffset = function(widget,conf,status,chValue)
  ...
  if conf.screenWheelChannelId > -1 then
    -- SW up
    if chValue < -600 then
      local offset = math.min(status.messageOffset + 21, math.max(0,status.messageCount - 21))
      if offset >= (status.messageCount - 21) then
        status.messageAutoScroll = true
      else
    ...
    if chValue > 600 then
      status.messageAutoScroll = false
      return math.max(0,math.max(status.messageCount - 200,status.messageOffset - 21))
    end
...
local function drawMsgDialog(msg)
  ...
  lcd.setColor(CUSTOM_COLOR,WHITE)
  lcd.drawFilledRectangle(100,160, 600, 160, CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,utils.colors.bars2)
  lcd.drawFilledRectangle(100+2,160+2, 600-4, 160-4, CUSTOM_COLOR)
  lcd.setColor(CUSTOM_COLOR,utils.colors.white)
  lcd.drawText(math.floor(LCD_W/2), 205, msg, DBLSIZE+CUSTOM_COLOR+CENTER)
end
...
local screenByPageMapping = {1,1,1,4,1,1,2,3}
...
local function drawFullScreen(widget)
  ...
  if not (resetPending or resetLayoutPending or loadConfigPending) then
    ...
    if drawStatus then
      libs.drawLib.drawFailsafe(197, 0)
      libs.drawLib.drawArmStatus(220, 100)
    end
  else
...
local function drawScreen(widget, event, touchState)
    -- horizontal screen 800x480
    if widget.zone.h < 475 then
