...
function hudLib.drawHud(widget,x,y,w,h)
  ...
  do
      yy = startY + (ii*step) + offset - step
      if yy >= startY and yy < endY then
        lcd.drawLine(342, yy+3, 342 + 4 , yy+3)
        libs.drawLib.drawNumber(339,  yy, j, 0, FONT_S, status.colors.hudDashes, RIGHT)
      end
      ii=ii+1;
  end
  ...
  local varioH = varioSpeed/varioMax*48
  ...
  if status.terrainEnabled == 1 then
    ...
    lcd.drawFilledRectangle(286, 98, 62, 16)
  end
  ...
  if status.airspeedEnabled == 1 then
    ...
    lcd.color(status.colors.black)
    lcd.pen(SOLID)
    lcd.drawFilledRectangle(x, 98, 62, 16)
    libs.drawLib.drawText(x+62, 101, "GS", FONT_S, status.colors.white,RIGHT)
  end

  if (math.abs(speed) >= 10) then
    libs.drawLib.drawNumber(x+2, 64, speed, 0, FONT_XXL, status.colors.hudSideText)
    if status.airspeedEnabled == 1 then
      libs.drawLib.drawNumber(x+2, 95, hSpeed, 0, FONT_L, status.colors.white)
    end
  else
    libs.drawLib.drawNumber(x+2, 64, speed, 1, FONT_XXL, status.colors.hudSideText)
    if status.airspeedEnabled == 1 then
      libs.drawLib.drawNumber(x+2, 95, hSpeed, 1, FONT_L, status.colors.white)
    end
  end
  --]]
  ...
  if status.conf.enableWIND == true then
    lcd.color(status.colors.black)
    lcd.pen(SOLID)
    lcd.drawFilledRectangle(x, 132, 62, 18)
    libs.drawLib.drawText(x + 62, 137,"WS",FONT_S,status.colors.white,RIGHT)
    local wind = status.telemetry.trueWindSpeed*status.conf.horSpeedMultiplier*0.1
    if math.abs(wind) >= 10 then
      libs.drawLib.drawNumber(x+2,132,wind,0,FONT_L,status.colors.white)
    else
      libs.drawLib.drawNumber(x+2,132,wind,1,FONT_L,status.colors.white)
    end
  end
  ...
  if math.abs(vSpeed*status.conf.vertSpeedMultiplier*10) > 99 then --
    libs.drawLib.drawNumber(234, 118, vSpeed*status.conf.vertSpeedMultiplier, 0, FONT_XXL, status.colors.white, CENTERED)
  else
    libs.drawLib.drawNumber(234, 118, vSpeed*status.conf.vertSpeedMultiplier, 1, FONT_XXL, status.colors.white, CENTERED)
  end
  --]]
  -- pitch and roll

  libs.drawLib.drawNumber(234,94,status.telemetry.pitch,0,FONT_STD,status.colors.black, CENTERED)
  libs.drawLib.drawNumber(210,76,status.telemetry.roll,0,FONT_STD,status.colors.black, RIGHT)
