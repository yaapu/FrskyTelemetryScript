...
function drawLib.drawMessagesBar(widget,maxRows)
  ...
  lcd.color(status.colors.barBackground)
  lcd.pen(SOLID)
  lcd.drawFilledRectangle(0,272-yDelta,480, yDelta)
  -- messages
  lcd.font(FONT_STD)
  ...
  for i=0,offset-1 do
    ...
    lcd.drawText(1,272 - yDelta + (12*i), msg[1])
  end
  return 272 - yDelta
end
...
function drawLib.drawNoTelemetryData(widget)
  if not libs.utils.telemetryEnabled() then
    ...
    lcd.font(FONT_STD)
    lcd.drawText(240, 152, "Yaapu Telemetry Widget 1.0.0 beta1".."("..'141293d'..")", CENTERED)
  end
end
...
function drawLib.drawArmingStatus(widget)
  ...
  if not libs.utils.failsafeActive(widget) and status.timerRunning == 0 then
    if status.telemetry.statusArmed == 1 then
      drawLib.drawBitmap( 150, 46, "armed")
    else
      drawLib.drawBlinkBitmap(150, 46, "disarmed")
    end
  end
end
