...
local function createOnce(widget)
  ...
  libs.utils.pushMessage(7,"Yaapu Telemetry Widget v1.0.0 dev ("..'49615e8'..")")
...
local function loadLayout(widget)
  ...
  lcd.drawFilledRectangle(192,98, 400, 140)
  lcd.color(status.colors.white)
  lcd.drawRectangle(192,98, 400, 140,3)
  lcd.color(status.colors.white)
  lcd.font(FONT_XL)
  lcd.drawText(260, 145, "loading layout...")
...
local function paint(widget)
    ...
    lcd.pen(SOLID)
    lcd.drawFilledRectangle(0,0,784,316)
    ...
    if status.showMessages then
      --libs.drawLib.drawStatusBar(widget,13)
    ...
    if libs.utils.telemetryEnabled() == false then
      ...
      lcd.color(RED)
      libs.drawLib.drawBlinkRectangle(0,0,784,316,3)
    else
      -- arming status
    ...
    lcd.font(FONT_S)
    lcd.color(status.colors.white)
    lcd.drawText(784,316-45,string.format("F %.01fHz",fg_rate),RIGHT)
    lcd.drawText(784,316-30,string.format("B %.01fHz",bg_rate),RIGHT)
end

-- called when event is passed to the widget
local function event(widget, category, value, x, y)
    local kill = true
    local yMin = widget.screen == 1 and 240 or 280
    if widget.screen == 1 then
      if status.showMessages then
        status.showMessages = false
      elseif y > yMin then
    ...
    elseif widget.screen == 2 then
      if libs.drawLib.isInside(x,y,550,0,784,150) == true then
        status.mapZoomLevel = math.min(status.conf.mapZoomMax, status.mapZoomLevel+1)
      elseif libs.drawLib.isInside(x,y,550,150,784,316) == true then
...
local function configure(widget)
  local f
  local line = form.addLine("Screen Type")
  widget.screenField = form.addChoiceField(line, form.getFieldSlots(line)[0],  {{"default",1}, {"maps", 2}, {"plot",3}}, function() return widget.screen end,
    function(value)
      widget.screen = value
  ...
  form.addChoiceField(line, form.getFieldSlots(line)[0], {{"Ardupilot", 1}, {"Frsky FLVSS", 2}}, function() return status.conf.defaultBattSourceId end,
    function(value)
