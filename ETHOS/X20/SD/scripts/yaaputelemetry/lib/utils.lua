...
function utils.playSound(soundFile, skipHaptic)
  ...
  system.playFile("/audio/yaapu/"..status.conf.language.."/".. soundFile..".wav")
end
...
function utils.playSoundByFlightMode(flightMode)
  ...
  if status.frame.flightModes then
    if status.frame.flightModes[flightMode] ~= nil then
      ...
      system.playFile("/audio/yaapu/"..status.conf.language.."/".. string.lower(status.frame.flightModes[flightMode]) .. ((status.frameTypes[status.telemetry.frameType]=="r" or status.frameTypes[status.telemetry.frameType]=="b") and "_r.wav" or ".wav"))
    end
  end
end
...
function utils.calcFLVSSBatt(battIdx)
  ...
  print("batterySource",batterySource)
