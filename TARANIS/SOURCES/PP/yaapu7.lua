...
local function drawConfigMenuBars()
  ...
  lcd.drawText(0,0,"Yaapu X7 1.7.2",SMLSIZE+INVERS)
...
local function drawItem(idx,flags)
  if menuItems[idx][2] == 0 then
    if menuItems[idx][4] == 0 then
      lcd.drawText(99,7 + (idx-menu.offset-1)*7, "---",0+SMLSIZE+flags+menuItems[idx][8])
    else
      lcd.drawNumber(99,7 + (idx-menu.offset-1)*7, menuItems[idx][4],0+SMLSIZE+flags+menuItems[idx][8])
      lcd.drawText(lcd.getLastRightPos(),7 + (idx-menu.offset-1)*7, menuItems[idx][7],SMLSIZE+flags)
    end
  else
    lcd.drawText(99,7 + (idx-menu.offset-1)*7, menuItems[idx][5][menuItems[idx][4]],SMLSIZE+flags)
  end
end
...
local function formatMessage(severity,msg)
  if lastMessageCount > 1 then
    ...
    return string.format("%d:%s %s (x%d)", messageCount, string.sub("EMRALRCRTERRWRNNOTINFDBG",severity*3+1,severity*3+3), msg, lastMessageCount)
  else
    if #msg > 23 then
      msg = string.sub(msg,1,23)
    end
    return string.format("%d:%s %s", messageCount, string.sub("EMRALRCRTERRWRNNOTINFDBG",severity*3+1,severity*3+3), msg)
  end
end
...
local function drawX7RightPane(x,battVolt,cellVolt,current,battmah,battcapacity)
  ...
  drawNumberWithTwoDims(x+24, 27, 28, 36,cellVolt,"V",battsource,DBLSIZE+PREC2+flags,dimFlags,SMLSIZE)
  -- battery voltage
  drawNumberWithDim(x+1,29,29, battVolt,"V",INVERS+PREC1+SMLSIZE,INVERS+SMLSIZE)
  -- battery current
...
local function drawX7LeftPane(battVolt,cellVolt,current,battmah,battcapacity)
  ...
  local dimFlags = 0
  if showMinMaxValues == false then
    if cellVolt < menuItems[3][4] then
      flags = BLINK
      dimFlags = BLINK
    elseif cellVolt < menuItems[2][4] then
      dimFlags = BLINK+INVERS
    end  
  end
  drawNumberWithTwoDims(0, 27, 28, 36,cellVolt,"V",battsource,DBLSIZE+PREC2+flags,dimFlags,SMLSIZE)
  -- battery voltage
  drawNumberWithDim(41+1,29,29, battVolt,"V",INVERS+PREC1+SMLSIZE,INVERS+SMLSIZE)
  -- battery current
...
--
  ...
  if math.abs(speed) > 99 then -- 
    lcd.drawNumber((64)/2 + 9, LCD_H - 14, speed*0.1, SMLSIZE+RIGHT)
  else
  ...
  elseif showConfigMenu then
    ...
    if batt2sources.fc or batt2sources.vs then
      if showDualBattery == false then
        ...
        drawX7RightPane(0+64+1,getNonZeroMin(batt1,batt2),getNonZeroMin(cel1m,cel2m),curr,mah1+mah2,cap1+cap2)
      else
        ...
        drawX7RightPane(0+64+1,batt1,cel1m,curr1,mah1,cap1)
