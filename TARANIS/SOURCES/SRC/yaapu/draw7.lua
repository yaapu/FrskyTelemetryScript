...
local function drawFailSafe(showDualBattery,ekfFailsafe,battFailsafe)
  ...
  if ekfFailsafe > 0 then
    lcd.drawText(xoffset + 32 + 64/2 - 31, 20 + yoffset, " EKF FAILSAFE ", SMLSIZE+INVERS+BLINK)
  end
  if battFailsafe > 0 then
    lcd.drawText(xoffset + 32 + 64/2 - 31, 20 + yoffset, " BATT FAILSAFE ", SMLSIZE+INVERS+BLINK)
  end
end
